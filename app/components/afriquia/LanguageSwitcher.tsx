...
const LanguageSelect = () => {
  ...
  return (
    <div className="flex items-center">
      ...
      <div className="relative text-[#111111] dark:text-white inline-block items-center">
        {/* Trigger Button */}
        <div
        ...
        >
          ...
          <HiMiniChevronDown className="fill-[#111111] dark:fill-white" />
        </div>
        {isOpen && (
          <div className="absolute left-0 mt-2 w-full border border-[#111111] dark:border-white rounded-lg shadow-lg">
            {options.map((option) => (
              <div
                key={option.value}
                className={`px-4 py-2 cursor-pointer ${option.label == "En" ? "rounded-t-lg" : ""} ${option.label == "Ar" ? "rounded-b-lg" : ""}`}
