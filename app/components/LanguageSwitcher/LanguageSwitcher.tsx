...
const LanguageSelect = ({languageItems, languageItemsTextColor, iconColor, chevronColor, itemBackgroundHoverColor}: LanguageSelectProps) => {
        ...
        {isOpen && (
          <div className="absolute left-0 mt-2 w-full border border-gray-300 rounded-lg shadow-lg">
            {languageItems.map((option) => (
              <div
                key={option.value}
                className={`px-4 py-2 hover:bg-[#052337] cursor-pointer ${option.label == "En" ? "rounded-t-lg" : ""} ${option.label == "Ar" ? "rounded-b-lg" : ""}`}
