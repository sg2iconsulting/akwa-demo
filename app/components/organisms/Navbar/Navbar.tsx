...
import MenuItemsList from "../../atoms/MenuItemsList/MenuItemsList";
import ToggleWithIcon from "../../molecules/ToggleSwitchWithIcon/ToggleSwitchWithIcon";
import SelectItems from "../../molecules/SelectItems/SelectItems";
  ...
  return (
    <nav className="w-full">
      <div className="w-full p-10 flex justify-between ">
        <Image src="/logo/akwaLogo.png" className=" aspect-[120/30] w-[80px]" />
        <MenuItemsList
          menuItems={[
            {
              menuItem: "Home",
        ...
        <div className="lg:flex gap-8 hidden">
          ...
          />
          <ToggleWithIcon checked={true} onChange={() => setIsChecked(!isChecked)} />
          <Button className="w-[160px] py-2 bg-[#19A0BF] text-white font-bold rounded-full">
            contact
          </Button>
        </div>
        <Button
          onClick={() => setIsOpen(!isOpen)}
          className="text-white text-[20px] md:text-[24px] lg:hidden"
        >
          <GrMenu />
        </Button>
      </div>
